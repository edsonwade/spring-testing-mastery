package code.with.vanilson.coffeeservice.integration;
...
public class CoffeeIntegrationTest {
    ...
    public void testGetCoffeeById() throws Exception {
        ...
        mockMvc.perform(get("/coffee/{id}", coffeeIds.get(0)))
                ...
                .andExpect(header().string(HttpHeaders.LOCATION, "/coffee/" + coffeeIds.get(0)))
                .andExpect(header().string(HttpHeaders.ETAG, "\"1\""))

                // Validate the contents of the response
                .andExpect(jsonPath("$.id").value(coffeeIds.get(0)))
    ...
    public void testGetCoffeeByIdNotFound() throws Exception {
        ...
        mockMvc.perform(get("/coffee/" + coffeeIds.get(0) + 3))
    ...
    public void testCreateCoffee() throws Exception {
        mockMvc.perform(post("/coffee")
                ...
                // Validate the headers
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(header().string(HttpHeaders.LOCATION, "/coffee/" + (coffeeIds.get(0) + 3)))
                .andExpect(header().string(HttpHeaders.ETAG, "\"1\""))

                // Validate the contents of the response
                .andExpect(jsonPath("$.id").value(coffeeIds.get(0) + 3))
    ...
    void testDeleteCoffeeNotFound() throws Exception {
        // Execute our DELETE request
        mockMvc.perform(delete("/coffee/{id}", coffeeIds.get(0) + 3))
                .andExpect(status().isNotFound());
    }
}
